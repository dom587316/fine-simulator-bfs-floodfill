...
class Cell:
    def __init__(self):
        ...
        self.visited = False
        self.visible = False
        self.distance = -1  # Khoảng cách từ điểm xuất phát
...
def floodfill_micromouse(maze, start, animation):
    rows, cols = len(maze), len(maze[0])
    stack = [(start, 0)]
    visited = set()
    ...
    animation.shortest_path = find_shortest_path(maze, start, goal)
    animation.update_display()
...
class MazeAnimation:
    def __init__(self, maze):
        ...
        self.window = tk.Tk()
        self.window.title("Micromouse Flood Fill - Dark Mode")
...
floodfill_micromouse(maze, start, animation)
